...
var ServerDate = (function(serverNow) {
    ...
    var scriptLoadTime = Date.now(), // Remember when the script was loaded.

        // Remember the URL of this script so we can call it again during synchronization.
    ...
    // Everything is in the global function ServerDate.  Unlike Date, there is no need for a constructor because there aren't instances.
    ...
    ServerDate.stop = function () {
        clearTimeout(routine);
        clearInterval(synchronizationInterval);
    }

    // Populate ServerDate with the methods of Date's instances that don't change state.
    ...
    ["toString", "toDateString", "toTimeString", "toLocaleString",
        ...
        "toISOString", "toJSON"].forEach(function(method) {
            ServerDate[method] = function() {
                return new Date(ServerDate.now())[method]();
            };
        });

    // Because of network delays we can't be 100% sure of the server's time.  We do know the precision in milliseconds and make it available here.
    ...
    // After a synchronization there may be a significant difference between our clock and the server's clock.
    // Rather than make the change abruptly, we change our clock by adjusting it once per second by the amortizationRate.
    ServerDate.amortizationRate = 50; // ms

    // The exception to the above is if the difference between the clock and server's clock is too great (threshold set below).
    // If that's the case then we skip amortization and set the clock to match the server's clock immediately.
    ...
    Object.defineProperty(ServerDate, "synchronizationIntervalDelay", {
        ...
        set: function(value) {
            ...
            synchronizationInterval = setInterval(synchronize,ServerDate.synchronizationIntervalDelay);

            //log("Set synchronizationIntervalDelay to " + value + " ms.");
        }
    });

    // After the initial synchronization the two clocks may drift so we automatically synchronize again every synchronizationIntervalDelay.
    ...
    function setTarget(newTarget) {
        ...
        if (target)
            message += " (" + (newTarget > target ? "+" : "-") + " " + Math.abs(newTarget - target) + " ms)";

        target = newTarget;
        //log(message + ".");

        // If the target is too far off from the current offset (more than the amortization threshold) then skip amortization.
    ...
    function synchronize() {
        ...
        function requestSample() {
            ...
            // At the earliest possible moment of the response, record the time at which we received it.
        ...
        function processSample(serverNow) {
            ...
            // Take 10 samples so we get a good chance of at least one sample with low latency.
            if (iteration < 10) {
                iteration++;
                requestSample();
            }
    ...
    function offsetAdjustment () {
        // Don't let the delta be greater than the amortizationRate in either direction.
        var delta = Math.max(-ServerDate.amortizationRate, Math.min(ServerDate.amortizationRate, target - offset));
        ...
        routine = setTimeout(offsetAdjustment, (!delta && isSynchronized)?60000:1000);
    }
    ...
    // Not yet supported by all browsers (including Safari).  Calculate the precision based on when the HTML page has finished loading and begins to load this script from the server.
    ...
    // Amortization process.  Every second, adjust the offset toward the target by a small amount.
    routine = setTimeout(offsetAdjustment, 1000);
